...
- (void)viewDidLoad
{
    ...
    backgroundQueue = dispatch_queue_create("com.alexandremarques.Tweet.bgqueue2", NULL);
...
- (IBAction)alerta:(id)sender {
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle: @"Tweet"
                                                   message: @"Integraçào com o Twitter"
                                                  delegate: self
        ...
        [store requestAccessToAccountsWithType:twitterAccountType
                         withCompletionHandler:^(BOOL granted, NSError *error) {
                             if (!granted) {
                                 ...
                                 NSLog(@"Usuário rejeitou acesso a conta.");
                             }
                             else {
                                     ...
                                     if (granted && !error) {
                                         ...
                                         dispatch_sync(dispatch_get_main_queue(), ^{
                                             ...
                                             if (self.accounts.count == 0) {
                                                 [[[[UIAlertView alloc] initWithTitle:nil message:@"Adicione uma conta do Twitter nas configuraçòes do app" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] autorelease] show];
                                             }
                                             else {
                                                 // Let them select the account they want to use
                                                 UIActionSheet* sheet = [[[UIActionSheet alloc] initWithTitle:@"Selecione sua conta do Twitter:"
                                                                                                     delegate:self
                                         ...
                                         dispatch_sync(dispatch_get_main_queue(), ^{
                                             NSString* message = [NSString stringWithFormat:@"Ocorreu um erro ao acessar a conta : %@", [error localizedDescription]];
...
- (void)loadCoreDataTweets {
    ...
    if (!results) {
        ...
        NSLog(@"Ocorreu um grave erro, reencie o app!");
    }
    //display data
...
-(IBAction)postTweet:(id)sender {
    ...
    {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Desculpe" message:@"Verifique sua conexào de internet e se você possui uma conta do Twitter" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alertView show];
    }
}
